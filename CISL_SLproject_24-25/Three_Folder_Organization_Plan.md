# Three-Folder Organization Plan for LANG 2077

**Project:** LANG 2077 - Language Skills for Human-AI Partnership  
**Academic Year:** 2024-25  
**Coordinator:** Dr. Simon Wang  
**Assistant:** Cici Luo  

---

## 📁 **Three Separate Folders Overview**

### **1. CISL Service Learning Project Folder** ✅
- **Location:** Google Drive (Cici Luo has access)
- **Link:** [https://drive.google.com/drive/u/0/folders/1YvZ4pR9aCksSMgLRxSJGVrq63nxA1-H7](https://drive.google.com/drive/u/0/folders/1YvZ4pR9aCksSMgLRxSJGVrq63nxA1-H7)
- **Access:** Dr. Wang (Owner), Cici Luo (Editor)
- **Purpose:** Service Learning project documentation and materials
- **Status:** ✅ **ACTIVE** - Ready for collaboration

### **2. CISL LANG 2077 Course Folder** ✅
- **Location:** OneDrive/SharePoint (HKBU staff only)
- **Link:** [https://hkbuhk-my.sharepoint.com/my?id=%2Fpersonal%2Fsimonwang%5Fhkbu%5Fedu%5Fhk%2FDocuments%2FSummer%202025%2FCISL%2DLang2077%2Ddeliverables&ga=1](https://hkbuhk-my.sharepoint.com/my?id=%2Fpersonal%2Fsimonwang%5Fhkbu%5Fedu%5Fhk%2FDocuments%2FSummer%202025%2FCISL%2DLang2077%2Ddeliverables&ga=1)
- **Shared with:** Ashley of CISL
- **Purpose:** General course materials and documentation
- **Status:** ✅ **FOUND** - Dr. Wang has access, Cici Luo does not

### **3. Tin Ka Ping LANG 2077 Course Folder** 🔍
- **Location:** OneDrive/SharePoint (HKBU staff only)
- **Shared with:** Tin Ka Ping colleague via email
- **Purpose:** TKP-specific course materials and documentation
- **Status:** 🔍 **NEED TO LOCATE** - Link needs to be found and documented
- **Report Status:** ✅ **SUBMITTED** - Report already submitted to Tin Ka Ping Foundation
- **Financial Status:** $149,913.48 unspent balance to be returned to GEO

---

## 🎯 **Current Status & Action Items**

### **Immediate Tasks**
- [ ] **Locate CISL LANG 2077 OneDrive link** shared with Ashley
- [ ] **Locate Tin Ka Ping LANG 2077 OneDrive link** shared via email
- [ ] **Document all three folder links** in this organization plan
- [ ] **Create access instructions** for Cici Luo (Google Drive only)
- [ ] **Set up folder structure** in Google Drive for all three projects

### **Collaboration Strategy**
Since Cici Luo only has access to Google Drive:
1. **Primary collaboration** happens in Google Drive folder
2. **OneDrive folders** remain HKBU staff only
3. **Dr. Wang** manages OneDrive folders and syncs relevant materials to Google Drive
4. **Cici Luo** focuses on Google Drive organization and documentation

---

## 📋 **Folder Structure Plan**

### **Google Drive Main Folder Structure**
```
Google Drive: https://drive.google.com/drive/u/0/folders/1YvZ4pR9aCksSMgLRxSJGVrq63nxA1-H7
├── 01_CISL_ServiceLearning/
│   ├── 01_Visual_Documentation/
│   ├── 02_Student_Information/
│   ├── 03_Learning_Documentation/
│   ├── 04_Service_Deliverables/
│   ├── 05_Feedback/
│   └── 06_Additional_Materials/
├── 02_CISL_CourseMaterials/
│   ├── Course_Documents/
│   ├── Student_Work/
│   ├── Assessments/
│   └── Resources/
└── 03_TKP_CourseMaterials/
    ├── Course_Documents/
    ├── Student_Work/
    ├── Assessments/
    └── Resources/
```

### **OneDrive SharePoint Structure**
```
OneDrive: https://hkbuhk-my.sharepoint.com/my?id=%2Fpersonal%2Fsimonwang%5Fhkbu%5Fedu%5Fhk%2FDocuments%2FSummer%202025%2FCISL%2DLang2077%2Ddeliverables&ga=1
├── CISL-Lang2077-deliverables/
│   ├── Administrative_Documents/
│   ├── Assessment_Tools/
│   ├── Student_Materials/
│   └── Community_Feedback/
```

---

## 🔗 **Links to Locate and Document**

### **CISL LANG 2077 Course Folder** ✅
- **Shared with:** Ashley of CISL
- **Platform:** OneDrive/SharePoint
- **Access:** HKBU staff only (Dr. Wang has access, Cici Luo does not)
- **Status:** ✅ **FOUND**
- **Link:** [https://hkbuhk-my.sharepoint.com/my?id=%2Fpersonal%2Fsimonwang%5Fhkbu%5Fedu%5Fhk%2FDocuments%2FSummer%202025%2FCISL%2DLang2077%2Ddeliverables&ga=1](https://hkbuhk-my.sharepoint.com/my?id=%2Fpersonal%2Fsimonwang%5Fhkbu%5Fedu%5Fhk%2FDocuments%2FSummer%202025%2FCISL%2DLang2077%2Ddeliverables&ga=1)

### **Tin Ka Ping LANG 2077 Course Folder** 🔍
- **Shared with:** Tin Ka Ping colleague
- **Platform:** OneDrive/SharePoint
- **Access:** HKBU staff only
- **Status:** 🔍 **NEED TO LOCATE**
- **Report Status:** ✅ **SUBMITTED** to Tin Ka Ping Foundation
- **Financial Status:** $149,913.48 unspent balance (Project #11.42.4151.136536)
- **Action:** Search email correspondence with TKP colleague for folder link

### **Tin Ka Ping Report Form**
- **Found:** [Tin Ka Ping Character and Ethics Development Scheme Report](https://hkbuhk-my.sharepoint.com/:w:/r/personal/simonwang_hkbu_edu_hk/_layouts/15/Doc.aspx?sourcedoc=%7B76644BA3-E354-4E85-8AE3-DE5053CA5FAF%7D&file=%E7%94%B0%E5%AE%B6%E7%82%B3%E5%9F%BA%E9%87%91%E6%9C%83%20%E9%9D%92%E5%B9%B4%E5%93%81%E6%A0%BC%E5%9F%B9%E8%82%B2%E8%A8%88%E5%8A%83%E5%A0%B1%E5%91%8A%E8%A1%A8_2024-25_template_LANG2077%20-%20Copy.docx&action=default&mobileredirect=true)
- **Status:** ✅ **FOUND** - This might be related to the TKP folder

---

## 📧 **Email Search Strategy**

### **Search Terms for CISL Folder**
- "Ashley" + "CISL" + "LANG 2077"
- "OneDrive" + "CISL" + "LANG 2077"
- "SharePoint" + "CISL" + "LANG 2077"
- "folder" + "CISL" + "LANG 2077"

### **Search Terms for TKP Folder**
- "Tin Ka Ping" + "LANG 2077"
- "TKP" + "LANG 2077" + "OneDrive"
- "TKP" + "LANG 2077" + "SharePoint"
- "TKP" + "folder" + "LANG 2077"

---

## 🔄 **Workflow for Cici Luo**

### **What Cici Luo Can Access:**
- ✅ Google Drive folder (full access)
- ✅ GitHub repository (when invited)
- ✅ All documentation in this organization plan

### **What Cici Luo Cannot Access:**
- ❌ OneDrive/SharePoint folders (HKBU staff only)
- ❌ HKBU internal systems
- ❌ Direct access to Ashley or TKP colleagues

### **Collaboration Process:**
1. **Dr. Wang** locates and documents OneDrive links
2. **Dr. Wang** creates folder structure in Google Drive
3. **Cici Luo** organizes materials in Google Drive
4. **Dr. Wang** syncs relevant materials from OneDrive to Google Drive
5. **Both** collaborate on documentation and organization

---

## 📝 **Next Steps**

### **Immediate Actions (Dr. Wang)**
1. **Search email correspondence** for OneDrive links
2. **Document all three folder links** in this plan
3. **Create Google Drive folder structure** for all three projects
4. **Share updated organization plan** with Cici Luo

### **Follow-up Actions (Both)**
1. **Review and approve** folder structure
2. **Set up regular sync schedule** between OneDrive and Google Drive
3. **Establish communication protocol** for collaboration
4. **Begin organizing materials** according to the plan

---

## 📊 **Progress Tracking**

| Task | Status | Responsible | Notes |
|------|--------|-------------|-------|
| Locate CISL OneDrive link | 🔍 In Progress | Dr. Wang | Search email correspondence |
| Locate TKP OneDrive link | 🔍 In Progress | Dr. Wang | Search email correspondence |
| Create Google Drive structure | ⏳ Pending | Dr. Wang | After links are found |
| Document all links | ⏳ Pending | Dr. Wang | Update this plan |
| Begin collaboration | ⏳ Pending | Both | After setup complete |

---

**Last Updated:** [Current Date]  
**Next Review:** [Date]  
**Status:** 🔍 **IN PROGRESS** - Locating missing OneDrive links
